        ...
        <Grid container spacing={2}>
          <Grid item xs={12}>
            <Stack spacing={1}>
              ...
              <Typography variant="h6">
                Time: {dayjs.utc(reservation.start).format('DD/MM/YYYY')} {dayjs.utc(reservation.start).format('HH:mm')} -{' '}
                {dayjs.utc(reservation.end).format('HH:mm')}
              </Typography>
              <Typography variant="h6">
                Status: {reservation.isCancelled ? 'Cancelled' : dayjs.utc(reservation.end).isBefore(now) ? 'Expired' : 'Incoming'}
              </Typography>
            </Stack>
          </Grid>
          {postMsg && (
          ...
          {!dayjs.utc(reservation.end).isBefore(now) && !reservation.isCancelled && (
            <Grid item xs={12}>
