...
describe(`POST /api/${apiVersion}/register`, () => {
	test('should return 200 and register user successfully', async () => {
		const response = await request.post(`/api/${apiVersion}/register`).send({
	...
	test('should return 200 and register user successfully', async () => {
		...
		expect(response.body.success).toBe(true);
		expect(response.body.msg.isAuthenticated).toBe(true);
		expect(response.body.msg.email).toBe('test2@example.com');
	...
	test('should return 403 for non-admin trying to register admin', async () => {
		...
		await user.save();
		const loginResponse = await request.post(`/api/${apiVersion}/auth`).send({
			email: 'admin@example.com',
			password: 'password123',
		});
...
describe(`POST /api/${apiVersion}/auth`, () => {
	test('should return 200 and log in user successfully', async () => {
		...
		expect(response.status).toBe(200);
		expect(response.body.msg.isAuthenticated).toBe(true);
		expect(response.body.msg.email).toBe('login@example.com');
		expect(response.body.msg.name).toBe('Login User');
		expect(response.body.msg.role).toBe('guest');
	});
...
describe(`POST /api/${apiVersion}/isAuthenticated`, () => {
	test('should return 200 and check if user is authenticated', async () => {
		...
		expect(response.status).toBe(200);
		expect(response.body.msg.isAuthenticated).toBe(true);
		expect(response.body.msg.email).toBe('check@example.com');
		expect(response.body.msg.name).toBe('Check User');
		expect(response.body.msg.role).toBe('guest');
	});

	test('should return 400 for nonlogged in user', async () => {
		const response = await request.post(`/api/${apiVersion}/isAuthenticated`);
...
describe(`POST /api/${apiVersion}/signout`, () => {
	test('should return 200 and sign out user successfully', async () => {
		...
		expect(responseCheckAuth.body.success).toBe(false);
		expect(responseCheckAuth.body.msg).toBe('srv_not_logged_in');
	});

	test('should return 200 but indicate non loggedin user', async () => {
		const response = await request.post(`/api/${apiVersion}/signout`);
